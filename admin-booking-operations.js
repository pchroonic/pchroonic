(()=>{
  ...
  function panel(){
    ...
    const el=document.createElement('div');el.id='bookingOpsPanel';el.className='admin-panel';el.innerHTML=`<div class="panel-head"><div><h2>Booking operations</h2><p>Control which appointments customers can request. Route density keeps an operating day in one postcode area (for example SE or SW) once its first booking is placed.</p></div><span class="crm-summary" id="bookingOpsSummary">Loading…</span></div>
      <div class="crm-toolbar booking-ops-toolbar">
        <label>Booking confirmation<select id="bookingOpsMode"><option value="manual">Manual approval</option><option value="automatic">Automatic for eligible bookings</option><option value="paused">Pause new customer bookings</option></select></label>
        ...
        <label class="consent"><input id="bookingOpsRoute" type="checkbox"><span>Keep each day within one postcode route zone</span></label>
      </div>
      <p>Automatic mode confirms Window Cleaning appointments after the customer accepts your final price. Mark unusual quotes for manual booking review in Quotes. Pausing stops new customer bookings; existing appointments stay in place.</p>
  ...
  function render(d){
    editable=!!d.canEdit;
    document.getElementById('bookingOpsMode').value=d.rules.confirmationMode||'manual';
    ...
    document.querySelectorAll('#bookingOpsPanel input, #bookingOpsPanel select').forEach(x=>x.disabled=!editable);
    const save=document.getElementById('bookingOpsSave');save.disabled=!editable;save.title=editable?'Save the current customer booking rules.':'You can view booking operations, but Settings permission is required to change them.';
    const status=document.getElementById('bookingOpsStatus');if(!editable)status.textContent='View only · Settings permission is required to edit.';
    document.getElementById('bookingOpsSummary').textContent=`${{manual:'Manual approval',automatic:'Automatic for eligible bookings',paused:'Bookings paused'}[d.rules.confirmationMode||'manual']} · ${d.rules.operatingDays.length} days · ${d.rules.enabledWindows.length} windows · ${d.rules.maxJobsPerDay}/day`;
  ...
  function rulesFromForm(){return {confirmationMode:document.getElementById('bookingOpsMode').value,horizonDays:Number(document.getElementById('bookingOpsHorizon').value),minimumNoticeHours:Number(document.getElementById('bookingOpsNotice').value),maxJobsPerDay:Number(document.getElementById('bookingOpsCapacity').value),routeDensityEnabled:document.getElementById('bookingOpsRoute').checked,operatingDays:[...document.querySelectorAll('#bookingOpsDays input:checked')].map(x=>Number(x.value)),enabledWindows:[...document.querySelectorAll('#bookingOpsWindows input:checked')].map(x=>x.value)}}
  ...
  async function save(){if(!editable)return;const btn=document.getElementById('bookingOpsSave'),status=document.getElementById('bookingOpsStatus');btn.disabled=true;status.textContent='Saving…';try{const d=await api('/api/admin-booking-operations',{method:'POST',body:JSON.stringify({action:'save',rules:rulesFromForm()})});render(d);status.textContent='Booking operations saved ✓'}catch(e){status.textContent=e.message}finally{btn.disabled=!editable}}
