(()=>{
  ...
  async function waitForSession(timeoutMs=10000){
    ...
    while(Date.now()-start<timeoutMs){
      try{if(typeof sb!=='undefined'&&sb?.auth){const {data}=await sb.auth.getSession();if(data?.session)return data.session}}catch{}
  ...
  async function claimPendingQuote(){
    ...
    const session=await waitForSession();if(!session)return;
  ...
  function enhanceScheduling(){
    if(typeof openQuoteSchedule==='function'&&!openQuoteSchedule.__bookingJourney){
      ...
      const wrapped=async id=>{await base(id);const field=$('#quoteScheduleAddress'),q=(typeof customerQuoteCache!=='undefined'?customerQuoteCache:[]).find(x=>x.id===id);if(field&&!field.value.trim()&&q?.inputs?.requestedAddress)field.value=q.inputs.requestedAddress;let note=$('#quoteScheduleJourneyNote');if(!note){note=document.createElement('p');note.id='quoteScheduleJourneyNote';note.className='quote-schedule-journey-note';note.textContent='Your accepted final price stays unchanged unless the job scope changes. The selected slot remains a request until Namdar confirms it.';$('#quoteScheduleDialog .modal-card h3')?.insertAdjacentElement('afterend',note)}};
      wrapped.__bookingJourney=true;openQuoteSchedule=wrapped;
    }
  }

  injectStyles();authContinuation();observeQuotes();enhanceScheduling();claimPendingQuote();
})();
