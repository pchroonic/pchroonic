(()=>{
  ...
  function inject(){
    ...
    anchor.insertAdjacentHTML('beforebegin',`<div class="admin-panel" id="${PANEL_ID}">
      <div class="panel-head"><div><h2>Daily address database growth</h2><p>Uses GetAddress postcode lookups to grow Namdar's private master address database automatically. Each postcode lookup can save many addresses.</p></div><span id="harvestState" class="crm-summary">Loading…</span></div>
      <div class="stat-grid"><div class="stat-card"><small>Cached addresses</small><strong id="harvestTotal">—</strong></div><div class="stat-card"><small>Today used</small><strong id="harvestUsed">—</strong></div><div class="stat-card"><small>Today remaining</small><strong id="harvestRemaining">—</strong></div><div class="stat-card"><small>Last success</small><strong id="harvestLast">—</strong></div></div>
      <div class="admin-form-grid"><label>Automatic daily harvest<select id="harvestEnabled"><option value="false">Off</option><option value="true">On</option></select></label><label>Maximum postcode lookups per day<input id="harvestCap" type="number" min="1" max="20" step="1" value="20"></label></div>
      ...
      <p id="harvestConfig" class="admin-note"></p><p id="harvestMessage" class="admin-note" aria-live="polite"></p><div id="harvestRuns"></div>
    </div>`);
  ...
  function render(d){
    const s=d.settings||{},dataset=d.dataset||{},provider=d.providerUsage;
  ...
  async function save(){
    ...
    try{const d=await api('/api/admin-address-harvest',{method:'POST',body:JSON.stringify({action:'settings',enabled:document.getElementById('harvestEnabled').value==='true',dailyLookupCap:Number(document.getElementById('harvestCap').value)})});render(d);message('Settings saved ✓')}catch(e){message(e.message)}finally{btn.disabled=false}
  }
  async function runNow(){
    const btn=document.getElementById('harvestRun');if(!confirm('Run the address harvest now? It will use only today\'s remaining GetAddress lookup allowance.'))return;
    btn.disabled=true;message('Harvesting new postcode addresses…');
    try{const d=await api('/api/admin-address-harvest',{method:'POST',body:JSON.stringify({action:'run-now'})});render(d);const r=d.result||{};message(r.skipped?`Skipped: ${r.reason||'no allowance'}`:`Run ${r.status||'finished'} · ${r.postcodesHarvested||0} postcodes · ${r.addressesCollected||0} addresses`)}catch(e){message(e.message)}finally{btn.disabled=false}
  }
