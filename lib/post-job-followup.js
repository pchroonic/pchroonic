...
function followUpEmail(booking,quote,feedbackUrl,reviewUrl){
  ...
  const publicButton=reviewUrl?`<p><a href="${escapeHtml(reviewUrl)}" style="display:inline-block;border:2px solid #173c32;color:#173c32;text-decoration:none;padding:10px 16px;border-radius:10px;font-weight:700">Leave an honest Google review</a></p><p style="font-size:13px;color:#68736f">Please describe your experience as it was — positive, neutral or negative. Namdar does not offer rewards for reviews.</p>`:'';
...
async function processOne(row){
  ...
  try{
    ...
    const content=followUpEmail(booking,quote,feedbackUrl,reviewUrl);
    ...
    if(sent?.ok){
      await db(`booking_notifications?id=eq.${encodeURIComponent(row.id)}`,{method:'PATCH',body:{status:'sent',provider_id:sent.data?.id||null,sent_at:new Date().toISOString(),error:null,updated_at:new Date().toISOString()}});
      return {sent:true};
    }
