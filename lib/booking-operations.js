...
function sanitizeRules(value={}){
  ...
  return {
    confirmationMode:['manual','automatic','paused'].includes(v.confirmationMode)?v.confirmationMode:'manual',
    horizonDays:int(v.horizonDays,7,21,DEFAULT_RULES.horizonDays),
...
async function loadRules(db){
  const rows=await db('site_settings?key=eq.booking_operations&select=value&limit=1');
...
async function quoteMapForBookings(db,bookings){
  ...
  const rows=await db(`quotes?id=in.(${ids.join(',')})&select=id,postcode,service_key`);
  return new Map((rows||[]).map(q=>[q.id,q]));
}

async function loadActiveBookings(db,rangeStart,rangeEnd){
  return db(`bookings?starts_at=lt.${encodeURIComponent(rangeEnd.toISOString())}&ends_at=gt.${encodeURIComponent(rangeStart.toISOString())}&status=in.(pending,confirmed)&select=id,quote_id,starts_at,ends_at,status&limit=1000`);
}
...
async function availabilityForQuote(db,quote,now=new Date()){
  ...
  if(rules.confirmationMode==='paused')return {rules,routeZone:zone,slots};
...
async function operationsPreview(db,now=new Date(),days=14){
  ...
  for(let offset=0;offset<Math.min(days,rules.horizonDays);offset++){
    ...
    out.push({date,operating:rules.confirmationMode!=='paused'&&rules.operatingDays.includes(weekday(date)),booked:rows.length,capacity:rules.maxJobsPerDay,routeZones:zones,openWindows:rules.confirmationMode==='paused'?0:Math.max(0,rules.enabledWindows.length-rows.length)});
  }
