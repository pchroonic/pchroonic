...
function queueTargetForRun(trigger='cron',requested=1){
  const n=Math.min(base.MAX_DAILY_LOOKUPS,Math.max(1,Number(requested)||1));
  return trigger==='manual'?n:Math.max(80,n*SERVICE_QUEUE_MULTIPLIER);
}
...
async function runHarvest(options={}){
  ...
  const target=queueTargetForRun(options.trigger,requested);
...
module.exports={...base,runHarvest,harvestStatus,seedServiceAreaQueue,queueTargetForRun};