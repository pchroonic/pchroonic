...
const { normalizeReminderDays } = require('./payment-policy');

const TRANSIENT_GATEWAY = new Set([502, 503, 504]);
const DEFAULT_CHUNK_SIZE = 50;
const LEGACY_OVERDUE_DAYS = Object.freeze([1, 8, 15, 29]);
...
function overdueReminderSchedule(invoice = {}) {
  const snapshot = invoice?.payment_policy_snapshot;
  if (!snapshot || snapshot.active !== true) return LEGACY_OVERDUE_DAYS.map((days, index) => ({ stage: index + 1, days, policyRevision: null }));
  const reminderDays = normalizeReminderDays(snapshot?.overdue?.reminderDays ?? snapshot?.overdue?.reminder_days);
  return reminderDays.map((days, index) => ({ stage: index + 1, days, policyRevision: Number.isFinite(Number(snapshot.revision)) ? Number(snapshot.revision) : null }));
}
...
async function scanBusinessFollowUpsBatched(deps = {}) {
  ...
  const invoices = await safeRows('overdue_invoices', () => db(`invoices?status=in.(issued,part_paid)&due_at=not.is.null&due_at=lt.${encodeURIComponent(nowIso)}&select=id,quote_id,booking_id,invoice_number,total,amount_paid,due_at,status,payment_policy_revision,payment_policy_snapshot,overdue_booking_hold_days,overdue_final_review_days&limit=500`));
  ...
  for (const invoice of invoices) {
    ...
    for (const item of overdueReminderSchedule(invoice)) {
      const due = businessUtcMorning(dueBase, item.days, 9);
      candidates.push({
        notification_type: 'invoice_overdue', entity_type: 'invoice', entity_id: invoice.id,
        event_key: businessEventKey(['invoice_overdue', invoice.id, dueBase.toISOString(), item.stage, item.days, item.policyRevision ?? 'legacy']),
        due_at: due.toISOString(), recipient_email: String(q.email).trim().toLowerCase(),
        status: 'pending', attempts: 0, metadata: {
          stage: item.stage, daysOverdue: item.days, dueAt: dueBase.toISOString(), policyRevision: item.policyRevision,
          bookingHoldAfterDays: invoice.overdue_booking_hold_days ?? invoice?.payment_policy_snapshot?.overdue?.bookingHoldAfterDays ?? null,
          finalReviewAfterDays: invoice.overdue_final_review_days ?? invoice?.payment_policy_snapshot?.overdue?.finalReviewAfterDays ?? null,
...
module.exports = { businessUtcMorning, businessEventKey, overdueReminderSchedule, dedupeCandidates, queueBusinessCandidates, scanBusinessFollowUpsBatched };
