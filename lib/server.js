...
function normalizeGoogleReviewUrl(value='') {
  ...
  try {
    ...
    return googleHost?url.href:'';
  } catch { return ''; }
}

async function publicReviewUrl() {
  const row=(await base.db('site_settings?key=eq.reviews&select=value&limit=1').catch(()=>[]))?.[0]||null;
  const value=row?.value&&typeof row.value==='object'?row.value:{};
  if(value.review_requests_enabled===false)return '';
  return normalizeGoogleReviewUrl(value.public_review_url);
}
...
module.exports = { ...base, accessTokenClaims, requireStaff, normalizeGoogleReviewUrl, publicReviewUrl, ensureInvoiceForBooking };
