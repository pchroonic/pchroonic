...
async function autocompletePostcode(postcode, apiKey) {
  const params = new URLSearchParams({ 'api-key': apiKey, all: 'true', template: TEMPLATE });
  const data = await providerRequest(`https://api.getAddress.io/autocomplete/${encodeURIComponent(postcode)}?${params}`, { timeoutMs: 10000 });
...
module.exports = { DATASET, BACKUP_BUCKET, MAX_DAILY_LOOKUPS, runHarvest, harvestStatus, rowsToCsv, exportDatasetRows, storageDownload };
