...
function normalizeDepositBands(value){
  ...
  return rows.slice(0,12).map((row,index)=>{
    ...
    return{index,minAmount,maxAmount,percent:clamp(row?.percent,1,100,DEFAULT_PAYMENT_POLICY.depositPercent),minimumDeposit:clamp(row?.minimumDeposit??row?.minimum_deposit,.5,100000,DEFAULT_PAYMENT_POLICY.minimumDeposit)};
...
function validatePaymentPolicy(value={}){
  ...
  if(policy.depositStrategy==='tiered'){
    ...
    policy.depositBands=bandValidation.bands;errors.push(...bandValidation.errors);
  }
...
function providerReadiness(env){
  const secret=String(env('STRIPE_SECRET_KEY','')||'').trim(),webhookSecret=String(env('STRIPE_WEBHOOK_SECRET','')||'').trim();
  const stripeConfigured=Boolean(secret),webhookConfigured=Boolean(webhookSecret),stripeMode=stripeKeyMode(secret),production=String(env('VERCEL_ENV','')||'').trim().toLowerCase()==='production';
  const liveReady=stripeConfigured&&webhookConfigured&&stripeMode==='live',testReady=stripeConfigured&&webhookConfigured&&stripeMode==='test';
  const ready=stripeConfigured&&webhookConfigured&&(!production||stripeMode==='live');
  let activationBlockReason='';
  if(!stripeConfigured)activationBlockReason='Connect a Stripe secret key before enabling online payments.';
  else if(!webhookConfigured)activationBlockReason='Configure and verify the Stripe webhook before enabling online payments.';
  else if(production&&stripeMode==='test')activationBlockReason='Production customer payments cannot be enabled with a Stripe test-mode key. Connect a live Stripe key and live webhook first.';
  else if(production&&stripeMode!=='live')activationBlockReason='Production customer payments require a recognised live Stripe secret key and verified live webhook.';
  return{stripeConfigured,webhookConfigured,ready,stripeMode,production,liveReady,testReady,activationBlockReason};
}
async function loadPaymentPolicy({db,env}){
  let stored={};try{stored=((await db('site_settings?key=eq.payments&select=value&limit=1'))?.[0]?.value)||{}}catch{}
...
function snapshotPaymentPolicy(policyValue,total=0){
  ...
  return{
    revision:policy.revision,active:policy.active,mode:policy.mode,depositStrategy:deposit.strategy,
    ...
    depositAmount:deposit.amount,initialPaymentRequired:policy.active&&policy.mode!=='optional',initialPaymentAmount:Number(initialPaymentAmount.toFixed(2)),
    allowFullPayment:policy.allowFullPayment,balanceDueHours:policy.balanceDueHours,
    overdue:{...policy.overdue,reminderDays:[...policy.overdue.reminderDays]},consumerMonetaryLateFees:false,commercialRecoveryMode:'manual_review'
  };
}
function paymentPolicyFromSnapshot(livePolicyValue,snapshot){
  const live=normalizePaymentPolicy(livePolicyValue),liveEffective=livePolicyValue?.effectiveActive===true;
  if(!snapshot||typeof snapshot!=='object')return{...live,mode:'optional',allowFullPayment:true,lockedDepositAmount:null,contractActive:false,legacyBooking:true,effectiveActive:liveEffective};
  const active=snapshot.active===true,hasLocked=snapshot.depositAmount!==null&&snapshot.depositAmount!==undefined&&Number.isFinite(Number(snapshot.depositAmount));
  return{
    ...live,
    ...
    lockedDepositAmount:hasLocked?Math.max(0,Number(snapshot.depositAmount)):null,
    contractActive:active,legacyBooking:false,effectiveActive:liveEffective
  };
}
function paymentRequirementMet(policy,paymentStatus='unpaid',state={}){
  ...
  const hasState=Number.isFinite(Number(state?.total))||Number.isFinite(Number(state?.net));
  if(!hasState)return policy.mode==='full_required'?paymentStatus==='paid':paymentStatus==='deposit_paid'||paymentStatus==='paid';
  const total=Math.max(0,Number(state.total)||0),net=Math.max(0,Number(state.net)||0);
  if(policy.mode==='full_required')return net>=total-.004;
  const hasLocked=policy.lockedDepositAmount!==null&&policy.lockedDepositAmount!==undefined&&Number.isFinite(Number(policy.lockedDepositAmount));
  const required=hasLocked?Math.max(0,Number(policy.lockedDepositAmount)):Math.min(total,Math.max(Number(policy.minimumDeposit||0),total*Number(policy.depositPercent||0)/100));
  return required<=.004||net>=required-.004;
}
function checkoutPlan({policy,total=0,net=0,outstanding=0,preferFull=false,forceBalance=false}){
  ...
  const percent=clamp(policy.depositPercent,1,100,DEFAULT_PAYMENT_POLICY.depositPercent),minimum=clamp(policy.minimumDeposit,.5,100000,DEFAULT_PAYMENT_POLICY.minimumDeposit);
  const hasLocked=policy.lockedDepositAmount!==null&&policy.lockedDepositAmount!==undefined&&Number.isFinite(Number(policy.lockedDepositAmount));
  const deposit=hasLocked?Math.min(due,Math.max(0,Number(policy.lockedDepositAmount))):Math.min(due,Math.max(minimum,invoiceTotal*percent/100));
...
function commercialLatePaymentPreview({principal=0,daysLate=0,baseRatePercent=0}){
  ...
  const interest=amount*(annualRate/100)*(days/365),recoveryCost=amount<1000?40:amount<10000?70:100;
...
function headlinePriceWithAllowance(amount,policy={}){
  const base=Math.max(0,Number(amount)||0),p=normalizePaymentPolicy(policy);if(!p.headlineAllowanceActive||base<=0)return base;
  const rate=Math.min(.15,Math.max(0,p.headlineAllowancePercent/100)),fixed=Math.max(0,p.headlineAllowanceFixed);return Number(((base+fixed)/(1-rate)).toFixed(2));
}

module.exports={DEFAULT_OVERDUE_POLICY,DEFAULT_PAYMENT_POLICY,normalizePaymentPolicy,validatePaymentPolicy,normalizeDepositBands,validateDepositBands,normalizeReminderDays,stripeKeyMode,providerReadiness,loadPaymentPolicy,resolveDepositRule,snapshotPaymentPolicy,paymentPolicyFromSnapshot,paymentRequirementMet,checkoutPlan,balanceDueAt,overdueStage,commercialLatePaymentPreview,headlinePriceWithAllowance};
