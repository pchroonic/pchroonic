...
function extractReference(lines){
  ...
  const precise=[
    /\binvoice\s*(?:#|no\.?|number|id)\s*:?\s*([A-Z0-9][A-Z0-9\-\/]{2,40}(?:\s+[0-9]{2,10})?)\b/i,
    /\breceipt\s*(?:#|no\.?|number|id)\s*:?\s*([A-Z0-9][A-Z0-9\-\/]{2,40})\b/i,
    /\bpayment\s+reference\s+(?:id|#|no\.?|number)\s*:?\s*([A-Z0-9][A-Z0-9\-\/]{2,60})\b/i,
    /\border\s*(?:#|no\.?|number|id)\s*:?\s*([A-Z0-9][A-Z0-9\-\/]{2,40})\b/i,
    /\b(?:transaction|txn)\s*(?:#|no\.?|number|id)\s*:?\s*([A-Z0-9][A-Z0-9\-\/]{2,40})\b/i
  ];
...
function extractDescription(lines=[],supplier=''){
  const asinIndex=lines.findIndex(x=>/\bASIN\s*:/i.test(x));
  if(asinIndex>0){
    const parts=[];
