...
function legalEntitySupplier(text=''){
  ...
  return m?clean(m[1]).replace(/^(?:sold by|supplied by|seller|supplier)\s+/i,''):'';
}
...
function specificTotal(text=''){
  const pats=[/\btotal\s+payable\s*(?:[:\-]\s*)?(?:£|\$|€|GBP\s*|USD\s*|EUR\s*)?(-?\d{1,6}(?:[,.]\d{3})*[.,]\d{2})\b/i,/\binvoice\s+total\s*(?:[:\-]\s*)?(?:£|\$|€|GBP\s*|USD\s*|EUR\s*)?(-?\d{1,6}(?:[,.]\d{3})*[.,]\d{2})\b/i,/\bamount\s+due\s*(?:[:\-]\s*)?(?:£|\$|€|GBP\s*|USD\s*|EUR\s*)?(-?\d{1,6}(?:[,.]\d{3})*[.,]\d{2})\b/i,/\bgrand\s+total\s*(?:[:\-]\s*)?(?:£|\$|€|GBP\s*|USD\s*|EUR\s*)?(-?\d{1,6}(?:[,.]\d{3})*[.,]\d{2})\b/i];
...
function extractReference(lines){
  ...
  const precise=[
    ...
    /\breceipt\s+([A-Z0-9][A-Z0-9\-\/]{2,40})\b/i,
...
function extractDescription(lines=[],supplier=''){
  ...
  if(asinIndex>0){
    ...
    for(let i=asinIndex-1;i>=0&&parts.length<4;i--){
      ...
      if(/^(?:\s*\((?:incl\.?|excl\.?)\s*VAT\)\s*)+$/i.test(line))continue;
      parts.unshift(line);
    }
