...
function labeledDate(text=''){
  ...
  for(const re of formats){const m=String(text).match(re);if(m){const d=extractDates(m[1])[0];if(d)return d}}
  return null;
}
...
function supplierFrom(lines,text=''){
  ...
  return legalEntitySupplier(text);
}
...
function extractReference(lines){
  ...
  const precise=[
    /\binvoice\s+(?:no\.?|number)\s*[:#]?\s*([A-Z0-9][A-Z0-9\-\/]{2,30}(?:\s+[0-9]{2,10})?)\b/i,
    /\breceipt\s+(?:no\.?|number)\s*[:#]?\s*([A-Z0-9][A-Z0-9\-\/]{2,30})\b/i,
    /\b(?:order|transaction|txn|ref(?:erence)?)\s*(?:no\.?|number|#|:)?\s*([A-Z0-9][A-Z0-9\-\/]{2,30})\b/i
  ];
  for(const re of precise){const m=text.match(re);if(m)return m[1].trim().replace(/\s+/g,'-')}
...
function extractReceipt(text='',rule=null){
  ...
  const vat=specificVat(normalized)||findLabeledAmount(lines,/\b(vat|tax|standard rate)\b/i,/\b(vat no|vat number|subtotal|total)\b/i);
