const CATEGORY_RULES=[
  ...
  ['software',/\b(adobe|microsoft 365|office 365|google workspace|canva|dropbox|software|saas|subscription)\b/i],
...
function extractReference(lines){
  for(const line of lines){
    const m=line.match(/\b(?:receipt|invoice|order|transaction|txn|ref(?:erence)?)\s*(?:no\.?|number|#|:)?\s*([A-Z0-9][A-Z0-9\-\/]{3,30})\b/i);
    if(m)return m[1];
  }
  return '';
}
function extractReceipt(text='',rule=null){
  ...
  const vat=findLabeledAmount(lines,/\b(vat|tax)\b/i,/\b(vat no|vat number|subtotal|total)\b/i);
  const baseCategory=categoryFor(normalized,total?.value),tax=taxTreatmentFor(normalized,baseCategory.category),method=paymentMethodFor(normalized);
  ...
  const confidence={supplier:supplier?.length?0.72:0.1,date:dateHit?0.86:0.1,amount:total?.value!==null&&total?.value!==undefined?(/total|amount/i.test(total.line)?0.93:0.62):0.1,vat:vat?0.82:0.4,category:learned?0.98:baseCategory.confidence,paymentMethod:learned?0.98:method.confidence};
  ...
  return{
    supplier:supplier||'',merchantKey:key,expenseDate:dateHit?.value||'',amount:total?.value??null,vatAmount:vat?.value??0,
