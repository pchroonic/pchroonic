...
async function retrieveCharge(secret,id,{expandBalanceTransaction=false}={}){return stripeRequest(`charges/${encodeURIComponent(id)}${expandBalanceTransaction?'?expand%5B%5D=balance_transaction':''}`,{secret})}
async function retrieveRefund(secret,id,{expandBalanceTransaction=false}={}){return stripeRequest(`refunds/${encodeURIComponent(id)}${expandBalanceTransaction?'?expand%5B%5D=balance_transaction':''}`,{secret})}
async function retrieveBalanceTransaction(secret,id){return stripeRequest(`balance_transactions/${encodeURIComponent(id)}`,{secret})}
function processorDetailsFromBalanceTransaction(txn){
  if(!txn||typeof txn!=='object')return null;
  const fee=Number(txn.fee),net=Number(txn.net),currency=String(txn.currency||'').toLowerCase();
  if(!Number.isFinite(fee)||!Number.isFinite(net)||!currency)return null;
  return{providerFee:Number((fee/100).toFixed(2)),providerNet:Number((net/100).toFixed(2)),providerFeeCurrency:currency,providerBalanceTransaction:String(txn.id||'').slice(0,180)||null};
}
async function balanceDetails(secret,value){
  if(value&&typeof value==='object')return processorDetailsFromBalanceTransaction(value);
  if(typeof value==='string'&&value)return processorDetailsFromBalanceTransaction(await retrieveBalanceTransaction(secret,value));
  return null;
}
async function retrievePaymentProcessorDetails(secret,paymentIntentId){
  const id=String(paymentIntentId||'');if(!id)return null;
  const pi=await stripeRequest(`payment_intents/${encodeURIComponent(id)}?expand%5B%5D=latest_charge.balance_transaction`,{secret});
  let charge=pi?.latest_charge;
  if(typeof charge==='string'&&charge)charge=await retrieveCharge(secret,charge,{expandBalanceTransaction:true});
  return balanceDetails(secret,charge?.balance_transaction);
}
async function retrieveRefundProcessorDetails(secret,refundOrId){
  let refund=refundOrId;
  if(typeof refundOrId==='string')refund=await retrieveRefund(secret,refundOrId,{expandBalanceTransaction:true});
  if(!refund||typeof refund!=='object')return null;
  return balanceDetails(secret,refund.balance_transaction);
}
...
async function readRawBody(req,{limit=1024*1024}={}){
  if(!req)throw stripeError('Webhook request is unavailable.',400);
  // Vercel's request.body helper is a lazy getter. Touching it first parses JSON and
  // consumes the exact bytes Stripe signed, so stream the IncomingMessage before any
  // body-helper access. The fallback below exists only for non-stream test adapters.
  ...
  throw stripeError('Webhook raw body is unavailable.',400);
}

module.exports={stripeRequest,checkoutIdempotencyKey,createCheckoutForm,createCheckoutSession,retrieveCheckoutSession,retrievePaymentIntent,retrieveCharge,retrieveRefund,retrieveBalanceTransaction,processorDetailsFromBalanceTransaction,retrievePaymentProcessorDetails,retrieveRefundProcessorDetails,parseStripeSignature,verifyStripeSignature,readRawBody};
