...
module.exports=async function handler(req,res){
  try{
    ...
    const [settingsRows,payments,invoices,expenses,jobCosts]=await Promise.all([
      ...
      db('payment_records?select=id,booking_id,invoice_id,direction,payment_kind,method,amount,provider_fee,provider_fee_currency,paid_at,created_at&order=paid_at.asc&limit=10000'),
    ...
    const yearPayments=(payments||[]).filter(x=>inRange(x.paid_at||x.created_at,bounds.start,bounds.end));
