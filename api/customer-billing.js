...
module.exports=async function handler(req,res){
  try{
    ...
    const rows=(invoices||[]).map(i=>{const b=bmap[i.booking_id]||{},q=qmap[i.quote_id]||{},paid=Number(i.amount_paid||0),total=Number(i.total||0);return{
      id:i.id,invoiceNumber:i.invoice_number,status:i.status,currency:i.currency,total,amountPaid:paid,outstanding:Number(Math.max(0,total-paid).toFixed(2)),issuedAt:i.issued_at,dueAt:i.due_at,paidAt:i.paid_at,notes:i.notes||'',
    ...
    return json(res,200,{ok:true,invoices:rows,stripeEnabled:Boolean(env('STRIPE_SECRET_KEY')),depositPercent:Math.max(1,Math.min(100,Number(env('NAMDAR_DEPOSIT_PERCENT','20'))||20))});
