...
module.exports=async function handler(req,res){
  try{
    ...
    const [quotes,bookings,projects,subscriptions,messages,tickets,ledger,invoices,payments,newsletter,privacyRequests,deletionRequests,chatSessions]=await Promise.all([
      safeRows(`quotes?or=(customer_id.eq.${uid},email.eq.${mail})&select=id,customer_id,customer_name,email,phone,postcode,service_key,inputs,automatic_estimate,final_price,status,created_at,updated_at,promo_code,promo_discount,reward_code,reward_discount,gross_estimate,net_estimate,photo_paths,expires_at,customer_response,customer_responded_at,customer_response_note,customer_quote_note,sent_at&order=created_at.desc`),
      safeRows(`bookings?customer_id=eq.${uid}&select=id,quote_id,starts_at,ends_at,address,status,payment_status,created_at,work_status,on_my_way_at,started_at,completed_at,customer_note,terms_version,booking_policy_version,booking_policy_accepted_at,early_service_requested_at,early_service_acknowledged,cancellation_window_hours,booking_policy_snapshot,payment_policy_revision,payment_policy_locked_at,payment_policy_snapshot,deposit_required&order=created_at.desc`),
