...
module.exports=async function(req,res){try{
  ...
  const block=await blockedRule(from),replyContext=!!thread||!!route.replyToken||!!inReplyTo;
  let assessment=block?{isSpam:true,score:100,reason:block.reason||(block.scope==='domain'?`Blocked domain @${block.value}`:`Blocked sender ${block.value}`)}:assessSpam({subject,body,headers,knownCustomer:!!customer,replyContext});
  if(!block&&!customer&&!replyContext&&await highFrequencyUnknownSender(from))assessment={isSpam:true,score:100,reason:'High-frequency unknown sender'};
  ...
  const spamSource=block?(block.scope==='domain'?'blocked_domain':'blocked_sender'):(assessment.isSpam?'automatic':thread?.spam_source||null);
