...
async function stripeCostForPayment(paymentIntentId){
  if(!paymentIntentId)return null;
  try{return await retrievePaymentProcessorDetails(env('STRIPE_SECRET_KEY'),paymentIntentId)}catch(e){console.warn('Stripe processing cost lookup pending:',e.message);return null}
}
async function stripeCostForRefund(refund){
  try{return await retrieveRefundProcessorDetails(env('STRIPE_SECRET_KEY'),refund)}catch(e){console.warn('Stripe refund cost lookup pending:',e.message);return null}
}
function providerFields(paymentIntentId,details){
...
async function processCheckoutSession(session){
  ...
  const paymentIntentId=String(session.payment_intent||''),processor=await stripeCostForPayment(paymentIntentId);
  const rows=await insertStripeRecord({booking_id:ctx.booking.id,invoice_id:ctx.invoice.id,customer_id:ctx.booking.customer_id||null,direction:'payment',payment_kind:kind,method:'stripe',amount:Number(amount.toFixed(2)),reference:paymentIntentId?`Stripe ${paymentIntentId.slice(0,80)}`:'Stripe Checkout',provider_reference:String(session.id),...providerFields(paymentIntentId,processor),paid_at:session.created?new Date(Number(session.created)*1000).toISOString():new Date().toISOString()});
...
async function processRefund(refund,paymentIntentId=''){
  ...
  const providerReference=`stripe_refund:${String(refund.id)}`,processor=await stripeCostForRefund(refund);
  const rows=await insertStripeRecord({booking_id:ctx.booking.id,invoice_id:ctx.invoice.id,customer_id:ctx.booking.customer_id||null,direction:'refund',payment_kind:'refund',method:'stripe',amount:Number(amount.toFixed(2)),reference:`Stripe refund ${String(refund.id).slice(0,80)}`,provider_reference:providerReference,...providerFields(piId,processor),paid_at:refund.created?new Date(Number(refund.created)*1000).toISOString():new Date().toISOString()});
  const state=await syncInvoicePaymentState(ctx.invoice.id);
  if(rows.length){
