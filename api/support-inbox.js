...
async function saveBlockRule(scope,value,ctx,reason='Blocked from Namdar email inbox'){
  const normalized=scope==='sender'?normalizeEmail(value):domainOf(`x@${String(value||'').replace(/^@/,'')}`);
  if(!normalized){const e=new Error('Could not determine a valid sender or domain.');e.status=400;throw e}
  const now=new Date().toISOString();
  const rows=await db(`support_inbox_blocklist?on_conflict=scope,value`,{method:'POST',prefer:'resolution=merge-duplicates,return=representation',body:{scope,value:normalized,reason:String(reason||'').slice(0,500)||null,active:true,created_by:ctx.user.id,updated_at:now}});
  return rows?.[0]||null
}
module.exports=async function(req,res){try{
  ...
  if(req.method==='GET'){
    if(String(queryParam(req,'blocklist',''))==='1'){
      if(!ctx.isAdmin)return json(res,403,{ok:false,error:'Only administrators can manage the inbox blocklist.'});
      const rules=await db('support_inbox_blocklist?active=eq.true&select=id,scope,value,reason,created_by,created_at,updated_at&order=scope.asc,value.asc&limit=500');
    ...
    return json(res,200,{ok:true,threads:enrichedThreads,unreadCount:threads.filter(t=>t.status!=='spam').reduce((a,t)=>a+Number(t.unread_count||0),0),staff});
  }
  if(req.method==='POST'){
    ...
    const t=await getThread(String(b.threadId||''),ctx);if(t.status==='spam')return json(res,400,{ok:false,error:'Restore this conversation from Spam before replying.'});const message=String(b.message||'').trim().slice(0,10000);if(!message)return json(res,400,{ok:false,error:'Write a reply first.'});
  ...
  if(req.method==='PATCH'){
    const b=parseBody(req),action=String(b.action||'read'),now=new Date().toISOString();
    if(action==='unblock_rule'){
      if(!ctx.isAdmin)return json(res,403,{ok:false,error:'Only administrators can change the inbox blocklist.'});
      const rule=(await db(`support_inbox_blocklist?id=eq.${encodeURIComponent(String(b.ruleId||''))}&active=eq.true&select=*&limit=1`))?.[0];if(!rule)return json(res,404,{ok:false,error:'Block rule not found.'});
      await db(`support_inbox_blocklist?id=eq.${encodeURIComponent(rule.id)}`,{method:'PATCH',body:{active:false,updated_at:now}});
      await auditLog(req,ctx,{action:'inbox.unblock',entityType:'support_inbox_blocklist',entityId:rule.id,summary:`Unblocked inbox ${rule.scope}`,before:rule,after:{...rule,active:false,updated_at:now}});
      return json(res,200,{ok:true})
    }
    const t=await getThread(String(b.threadId||''),ctx),patch={updated_at:now};
    if(action==='read')patch.unread_count=0;
    else if(action==='unread')patch.unread_count=t.status==='spam'?0:Math.max(1,Number(t.unread_count||0));
      ...
      if(!['awaiting_staff','awaiting_customer','closed','spam'].includes(b.status))return json(res,400,{ok:false,error:'Invalid status.'});
      patch.status=b.status;if(['closed','spam'].includes(b.status))patch.unread_count=0;
      if(b.status==='spam'){patch.spam_score=100;patch.spam_source='manual';patch.spam_reason='Marked as spam by Namdar staff'}
      else if(t.status==='spam'){patch.spam_score=0;patch.spam_source=null;patch.spam_reason=null}
    }
    ...
    else if(action==='block_sender'||action==='block_domain'){
      if(!ctx.isAdmin)return json(res,403,{ok:false,error:'Only administrators can block email senders or domains.'});
      const scope=action==='block_sender'?'sender':'domain',value=scope==='sender'?normalizeEmail(t.customer_email):domainOf(t.customer_email);
      if(!value)return json(res,400,{ok:false,error:'Could not determine a valid sender or domain.'});
      if(scope==='domain'&&PROTECTED_SHARED_DOMAINS.has(value))return json(res,400,{ok:false,error:'This is a shared/public email domain. Block the exact sender instead so legitimate customers are not affected.'});
      const rule=await saveBlockRule(scope,value,ctx,scope==='sender'?`Blocked sender ${value}`:`Blocked domain @${value}`);
      patch.status='spam';patch.unread_count=0;patch.spam_score=100;patch.spam_source=scope==='sender'?'blocked_sender':'blocked_domain';patch.spam_reason=scope==='sender'?`Sender ${value} is blocked`:`Domain @${value} is blocked`;
      if(scope==='sender')await db(`support_inbox_threads?customer_email=eq.${encodeURIComponent(value)}&status=neq.spam`,{method:'PATCH',body:{status:'spam',unread_count:0,spam_score:100,spam_source:'blocked_sender',spam_reason:`Sender ${value} is blocked`,updated_at:now}}).catch(()=>null);
      await auditLog(req,ctx,{action:`inbox.${action}`,entityType:'support_inbox_blocklist',entityId:rule?.id||t.id,summary:`Blocked inbox ${scope}`,metadata:{scope,value}});
    }
