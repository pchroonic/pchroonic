...
async function otherActiveOwners(id){
  const [owners,admins]=await Promise.all([
    db('staff_access?role_key=eq.owner&active=eq.true&select=user_id').catch(()=>[]),
    db('profiles?role=eq.admin&select=id,account_status').catch(()=>[])
  ]);
  const activeAdmins=new Set((admins||[]).filter(x=>!x.account_status||x.account_status==='active').map(x=>x.id));
  return (owners||[]).filter(x=>x.user_id!==id&&activeAdmins.has(x.user_id));
}
...
async function accessPlan({profileRole,requestedRoleKey,directPermissions,existingAccess=null,actorOwner=false}){
  ...
  return {roleKey:role?.key||null,role,permissions};
}
...
module.exports=async function(req,res){try{
  if(req.method==='GET'){
    ...
    const access=scope==='staff'?await db('staff_access?select=user_id,permissions,job_title,active,role_key'):[];
  ...
  if(req.method==='POST'){
    ...
    try{
      ...
      if(plan)await db('staff_access',{method:'POST',prefer:'resolution=merge-duplicates',body:{user_id:user.id,permissions:plan.permissions,job_title:b.jobTitle||plan.role?.name||(role==='admin'?'Administrator':'Staff'),active:true,created_by:staff.user.id,role_key:plan.roleKey}});
      await auditLog(req,staff,{action:'user.invite',entityType:'profile',entityId:user.id,summary:`Invited ${role} account for ${fullName}`,after:{email,full_name:fullName,phone:b.phone||null,postcode:b.postcode?postcode(b.postcode):null,role,account_status:status,job_title:plan?(b.jobTitle||plan.role?.name||(role==='admin'?'Administrator':'Staff')):null,access_role:plan?.roleKey||null,permissions:plan?.permissions||null},metadata:{credentialDelivery:'supabase_invitation',temporaryPassword:false}});
  ...
  if(req.method==='PATCH'){
    ...
    const roleOrPermissionChange=desiredRole!==existing.role||b.permissions!==undefined||b.jobTitle!==undefined||b.staffActive!==undefined||b.accessRoleKey!==undefined;
    const staff=await requireStaff(req,(['staff','admin'].includes(existing.role)||['staff','admin'].includes(desiredRole)||roleOrPermissionChange)?'staff':'customers'),actorOwner=await isOwner(staff);
    const targetOwner=existing.role==='admin'&&existingAccess?.role_key==='owner';
    ...
    if(targetOwner&&(!existing.account_status||existing.account_status==='active')&&(desiredRole!=='admin'||desiredStatus!=='active'||desiredRoleKey!=='owner')){
      if(!(await otherActiveOwners(id)).length)return json(res,409,{ok:false,error:'Namdar must keep at least one other active Owner before this Owner can be demoted, suspended or reassigned.'});
    }
    ...
    if(plan){
      await db('staff_access',{method:'POST',prefer:'resolution=merge-duplicates',body:{user_id:id,permissions:plan.permissions,job_title:b.jobTitle===undefined?(existingAccess?.job_title||plan.role?.name||(desiredRole==='admin'?'Administrator':'Staff')):(b.jobTitle||plan.role?.name||(desiredRole==='admin'?'Administrator':'Staff')),active:b.staffActive===undefined?(existingAccess?.active!==false):b.staffActive!==false,created_by:existingAccess?.created_by||staff.user.id,role_key:plan.roleKey,updated_at:new Date().toISOString()}});
    }else if(existingAccess){await db(`staff_access?user_id=eq.${encodeURIComponent(id)}`,{method:'DELETE'}).catch(()=>null)}
    const updatedProfile=await targetProfile(id),updatedAccess=plan?await targetStaffAccess(id):null;
    await auditLog(req,staff,{action:'user.update',entityType:'profile',entityId:id,summary:`Updated ${updatedProfile?.full_name||existing.full_name||'user'} account`,before:{...existing,staff_access:existingAccess},after:{...updatedProfile,staff_access:updatedAccess},metadata:{roleOrPermissionChange,emailIdentityChanged:false,accessRoleChanged:(existingAccess?.role_key||null)!==(updatedAccess?.role_key||null)}});
  ...
  if(req.method==='DELETE'){
    ...
    const staff=await requireStaff(req,['staff','admin'].includes(p.role)?'staff':'customers'),actorOwner=await isOwner(staff);
    const access=await targetStaffAccess(id),targetOwner=p.role==='admin'&&access?.role_key==='owner';
    if(p.role==='admin'&&!actorOwner)return json(res,403,{ok:false,error:'Only an Owner can delete an Administrator account.'});
    if(id===staff.user.id)return json(res,400,{ok:false,error:'You cannot delete the account you are currently using.'});
    if(targetOwner&&(!p.account_status||p.account_status==='active')&&!(await otherActiveOwners(id)).length)return json(res,409,{ok:false,error:'Namdar must keep at least one active Owner. Add and verify another Owner before deleting this account.'});
    ...
    await auditLog(req,staff,{action:'user.delete',entityType:'profile',entityId:id,summary:`Permanently deleted ${p.full_name||p.email||p.role||'user'} account`,before:{...p,staff_access:access},metadata:{targetRole:p.role,targetAccessRole:access?.role_key||null}});
